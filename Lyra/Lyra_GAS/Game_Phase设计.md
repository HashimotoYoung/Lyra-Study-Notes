...
- **每一个 "Phase" 被封装成一个GA(`ULyraGamePhaseAbility`)**
...
游戏阶段子系统, 继承自 `UWorldSubsystem`

- **Created per-world:** 当执行 `ULyraSystemStatics::PlayNextGame()` 时会调用 `ServerTravel()`, 从而会创建新的 `ULyraGamePhaseSubsystem` 实例
- 使用了 *nested private struct definition*
- [使用了 delegate-based Observer 模式](../Lyra_Tips.md#1-delegate-based-观察者模式)
```cpp
