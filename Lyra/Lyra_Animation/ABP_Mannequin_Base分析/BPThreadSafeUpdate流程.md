...
`IsCrouching` 
`GameplayTag_IsADS` 
`float AdditiveLeanAngle` ***W***; [当前帧倾斜角度](General.md#additiveleanangle--float-);
...
`float CardinalDirectionDeadZone` [基本方位](General.md#carinaldirectiondeadzone--float)
`RootYawOffset` [模型旋转偏移](./Base.md#rootyawoffset--float-);
`HasVelocity`  ***W***
...
4. 更新Local速度的**相对移动方位Enum**
   SET `LocalVelocityDirection` = **`SelectCardinalDirectionFromAngle( LocalVelocityDirectionAngleWithOffset... )`** [方法说明](./Base.md#selectcardinaldirectionfromangle)
   // 接下来计算NoOffset版本方位
...
- `WorldRotation` 此方法必须在UpdateRotation之后调用
  ...
  `HasAcceleration` ***W***
  `Enum CardinalDirectionFromAcceleration` ***W***, [基本加速度方位](./Base.md#carinaldirectionfromacceleration--enum)
...
##### 相关机制: 
- [其它 Set RootYawOffsetMode 的位置](./SM_Locomotion分析.md#state-要点分析)

##### 相关属性: 

-   `YawDeltaSinceLastUpdate`
  `Enum RootYawOffsetMode` ***W***, 有三种状态
  `float RootYawOffset` ***W***, [核心属性](./Base.md#rootyawoffset--float-star)
  `AimYaw` ***W***
