...
### 1.1 Idle 状态
[Layer层实现](../ABP_ItemAnimLayerBase%20分析/FullBody_IdleState实现.md)
##### 相关属性:
- `TurnYawCurveValue`
  `RootYawOffsetMode`
  `RootYawOffset` [核心属性](./Base.md#rootyawoffset--float-star)
...
3. **if** `previousTurnYawCurveValue != 0` 
    ...
    CALL [SetRootYawOffset( targetRootYawOffset )](./BPThreadSafeUpdate流程.md#setrootyawoffsetfloat-inrootyawval-实现)
    // **逐渐归零 RootYawOffset, 进而驱动下半身旋转**

---

### 1.2 Start 状态
[Layer层实现](../ABP_ItemAnimLayerBase%20分析/FullBody_StartState实现.md)
##### 相关属性:
- `RootYawOffset`
  `RootYawOffsetMode`
...
// [为什么要设为Hold](./Base.md#rootyawoffsetmodeenum)

---

### 1.3 Stop 状态
[Layer层实现](../ABP_ItemAnimLayerBase%20分析/FullBody_StopState实现.md)
##### 相关属性:
- `RootYawOffsetMode`
...
---
### 1.4 Cycle 状态
[Layer层实现](../ABP_ItemAnimLayerBase%20分析/FullBody_CycleState实现.md)
##### Transitions:
...
处理人物急转身相关动画, [Layer层实现](../ABP_ItemAnimLayerBase%20分析/FullBody_PivotState实现.md)
...
- `LocalVelocity2D`
  ...
  // 状态专属, 初始速度方向枚举
  **`LastPivotTime`** [核心属性](./Base.md#lastpivottime--float)
  // **由Layer层Set的值**
...
## 2. 跳跃 Graph
[跳跃Layer层实现](../ABP_ItemAnimLayerBase%20分析/FullBody_JumpAndFall实现.md)
